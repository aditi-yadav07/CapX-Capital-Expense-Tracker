...
function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const resTx = await axios.get('https://cap-x-capital-expense-tracker.vercel.app/api/transactions', config);
      setTransactions(resTx.data);

      const resCat = await axios.get('https://cap-x-capital-expense-tracker.vercel.app/api/transactions/categories', config);
      ...
      const resBudget = await axios.get('https://cap-x-capital-expense-tracker.vercel.app/api/budgets/status', config);
  ...
  const handleFastUpiPayment = async (e) => {
    ...
    try {
      ...
      await axios.post('https://cap-x-capital-expense-tracker.vercel.app/api/payments/verify-record', payload, config);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const res = await axios.post('https://cap-x-capital-expense-tracker.vercel.app/api/chat/message', { message: userMessage }, config);
  ...
  const handleAddTransaction = async (e) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.post('https://cap-x-capital-expense-tracker.vercel.app/api/transactions', {
  ...
  const handleSetBudget = async (e) => {
    ...
    try {
      ...
      await axios.post('https://cap-x-capital-expense-tracker.vercel.app/api/budgets', {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Purge this transaction record from main mainframe?')) {
      try {
        const config = { headers: { Authorization: `Bearer ${token}` } };
        await axios.delete(`https://cap-x-capital-expense-tracker.vercel.app/api/transactions/${id}`, config);
        fetchData();
      } catch (err) {
